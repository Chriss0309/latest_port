  ...
  return (
    <motion.div
      animate={{
        ...
        boxShadow: visible
          ? "0 0 24px rgba(0, 0, 0, 0.3), 0 0 0 1px rgba(255, 255, 255, 0.1) inset"
          : "none",
      ...
      className={cn(
        ...
        visible && "bg-neutral-900/80 border border-white/10",
        className,
      )}
    >
      {children}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-neutral-900/95 backdrop-blur-md px-4 py-8 shadow-xl border border-white/10",
            className,
          )}
        >
          {children}
...
export const NavbarButton = ({
  ...
  const variantStyles = {
    ...
    dark: "bg-neutral-800 text-white border border-white/20 hover:bg-neutral-700",
    gradient:
